...
Ex: `missing_data = df.isnull()`
`missing_data.head(5)`   
#check if they're true or false,"True" stands for missing value, while "False" stands for not missing value.
...
Note: Whole columns should be dropped only if most entries in the column are empty. 
...
8.replace the missing 'num-of-doors' values by the most frequent 
